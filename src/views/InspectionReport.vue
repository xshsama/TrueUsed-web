<template>
    <div class="min-h-screen bg-transparent pb-10">
        <div class="mx-auto max-w-[1480px] px-8 py-6">
                ...
                <div class="relative z-10 flex items-end justify-between gap-8">
                    <div>
                        ...
                        <p class="mt-3 max-w-2xl text-sm leading-7 text-slate-100/72">
                            这页现在直接挂在主 navbar 下，不再单独跳到一套独立页面。你可以像浏览首页和榜单一样，在主壳层里查看报告、筛选状态和打开详情。
                        </p>
                    </div>
            ...
            <section class="mt-6 grid grid-cols-[minmax(0,1fr)_300px] gap-6">
                <div class="space-y-6">
                    <div
                ...
                <aside class="space-y-6">
                    <div
                        ...
                        <div class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Desk Insight</div>
                        <h3 class="mt-3 text-2xl font-black text-slate-950">报告说明</h3>

                        <div class="mt-5 space-y-3">
                            <div v-for="item in sideNotes" :key="item"
                        ...
                        <div class="mt-6 rounded-[24px] bg-slate-950 p-5 text-white">
                            <div class="text-xs uppercase tracking-[0.18em] text-white/55">页面结构</div>
                            <div class="mt-2 text-lg font-bold">已并入主导航壳层</div>
                            <div class="mt-2 text-sm leading-6 text-white/68">
                                现在从顶部 navbar 进入验货报告时，不会再切到一套单独头部页面，浏览路径和首页、捡漏榜保持一致。
                            </div>
                        </div>
                    </div>
                </aside>
...
const sideNotes = computed(() => {
    return [
        '报告页现在属于主桌面导航体系，路径层级和其他核心页面一致。',
        '报告详情依然单独打开，避免把长表单、时间线和图片证据全部塞进列表页。',
        '你可以先在这里筛报告，再进入详情，不需要先跳到另一套页面框架。'
    ];
});
        ...
        reports.value = (res || [])
            ...
            .map(item => {
                ...
                return {
                    ...
                    inspector: `工号${100 + Math.floor(Math.random() * 50)}`,
                    location: '上海',
